...
# Настройка БД: PostgreSQL (Neon) или SQLite для локальной разработки
...
# Пути для файлов (на Render используем /tmp)
basedir = os.path.abspath(os.path.dirname(__file__))
...
def api_statistics():
    user = get_current_user()
    return jsonify({
        'files_count': user.get_file_count(),
        'documents_count': user.get_document_count(),
        'total_size_bytes': user.get_total_storage_used(),
        'total_size_mb': round(user.get_total_storage_used() / (1024 * 1024), 2)
    })
