...
def index():
    ...
    return render_template('index.html', user=user, files_count=files_count, 
                         users_count=users_count, recent_files=recent_files)
...
def register():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            save_to_csv('users_log.csv', {
                'timestamp': datetime.now().isoformat(),
                'action': 'register',
                'username': username,
                'email': email
            }, ['timestamp', 'action', 'username', 'email'])
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                save_to_csv('files_log.csv', {
                    'timestamp': datetime.now().isoformat(),
                    'action': 'upload',
                    'user_id': session['user_id'],
                    'filename': original_filename,
                    'file_size': file_size
                }, ['timestamp', 'action', 'user_id', 'filename', 'file_size'])
...
def list_files():
    user = get_current_user()
    user_files = File.query.filter_by(user_id=user.id).order_by(File.uploaded_at.desc()).all()
    public_files = File.query.filter_by(is_public=True).order_by(File.uploaded_at.desc()).all()
...
def delete_file(file_id):
    ...
    try:
        ...
        save_to_csv('files_log.csv', {
            ...
            'file_id': file_id
        }, ['timestamp', 'action', 'user_id', 'filename', 'file_id'])
...
def create_document():
    if request.method == 'POST':
        ...
        try:
            ...
            save_to_csv('documents_log.csv', {
                'timestamp': datetime.now().isoformat(),
                'action': 'create',
                'user_id': session['user_id'],
                'document_id': new_doc.id,
                'title': title
            }, ['timestamp', 'action', 'user_id', 'document_id', 'title'])
